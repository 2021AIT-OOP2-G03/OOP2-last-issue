...
### UIの不具合の改善,アプリ内で発生するバグを修正する(氏家君)
- ノーツの横幅を固定幅(200)からウィンドウサイズ(Window.size[0]/(2*len(melody)))に合わせるよう変更してください。
- HomeScreenにて、難易度がデフォルトが何も設定されてない状態です。このままゲームを開始するとゲームが落ちるバグが発生しますので、そこに初期値を与えるか例外処理を書くなどの処置を取ってバグを解消してください。
- 些細なことでも構いませんので、困ったことがありましたら気軽に松井に相談してください。
